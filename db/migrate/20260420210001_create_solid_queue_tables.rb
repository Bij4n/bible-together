class CreateSolidQueueTables < ActiveRecord::Migration[8.1]
  def change
    ...
    # All six child tables are newly created in this migration with zero rows,
    # so the FK validation scan has nothing to lock or scan. strong_migrations
    # can't tell the tables are brand-new, hence the assertion.
    safety_assured do
