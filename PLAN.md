...
**Sprint 14 closed.** Shared flashes, settings, groups (5 files), admin (3 files), manuscript-token + Cinzel/EB Garamond font-loader deletion. Five commits. The design pivot's migration phase is complete — every user-facing surface is on the modern token set; `application.css` no longer carries any manuscript tokens or inert rules. Sprint 15 picks up: homepage content rewrite (hero / features / about), Bitcoin donation integration, SMTP provider wiring. No visual-migration work crosses into Sprint 15.
