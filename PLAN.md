...
- **2026-04-24 Sprint 15 open.** First sprint operating under rules 7 / 8 / 9, first sprint with branch protection on main. Locked decisions: **Bitcoin processor = OpenNode** (simplest webhook pattern, 1% fee, no monthly cost; BTCPay Server self-hosted rejected for this sprint due to ops burden — revisit in a later sprint if sovereignty becomes priority). **SMTP = Resend** (3000/mo free tier, modern DX, DNS verification for `bible-together.org` required; Postmark fallback if Resend deliverability disappoints). Jurisdiction / entity status for donations flagged as a blocker before the donation implementation commit, not blocking the brief. Semantic search claim on the homepage gated on the embedding pserv being verified live — if it's still down, drop from the features grid until a follow-up. No "open source" language until the repo is public; no bare "accessibility-first" claim — use "WCAG AA tested" with the axe-clean reference.
...
**Goal:** close out the v1 product surface. Rewrite the homepage content (hero, features, how-it-works, about, donation CTA) on top of the Sprint 13 visual shell. Ship a Bitcoin donation flow — static address-only is too weak for a real site, processor-integrated with webhook confirmation is the right bar. Wire an SMTP provider so Devise password reset stops silent-failing. No visual-system work; the design pivot (Sprints 12-14) closed that.

**Decisions locked in this brief** (user confirms or vetoes before implementation commit):

- **Bitcoin processor: OpenNode.** Rationale: simplest webhook-integration pattern (one create-invoice endpoint + one webhook receiver), 1% fee with no monthly cost, supports both on-chain and Lightning. Merchant KYC is required but that's a one-time setup, not recurring friction. Tradeoff honestly called out: OpenNode is a third-party dependency, which violates this repo's self-hosted-first preference (established by the font self-hosting in Sprint 14 and the anti-Google-Fonts rule). The fully-sovereign alternative — BTCPay Server self-hosted — requires a separate running service with its own uptime / backup / wallet-management burden; that's out of scope for a "ship donations" sprint. If sovereignty becomes a priority later, migrate to BTCPay in a dedicated sprint; the `DonationsController` + webhook shape would port.
- **SMTP provider: Resend.** Rationale: 3000 emails / month free tier (more than Postmark's 100/month sandbox), modern DX, clean Rails setup via ActionMailer SMTP config. DNS verification required for `bible-together.org` sender — SPF + DKIM — but that's expected for any provider. Fallback pick if Resend deliverability disappoints: Postmark ($15/mo for 10k transactional emails; best-in-class reputation but a paid baseline).
- **Jurisdiction / entity status for donations** — **flagged as a blocker before the donation implementation commit**, not this brief. User resolves separately; affects merchant KYC approval, receipt content, and tax-treatment disclosure on the donate page. Brief drafts the shape assuming a personal project with a simple "donations support the project" disclaimer; if user's answer changes that (nonprofit entity, different jurisdiction with specific disclosure rules), the donate-page copy gets revised before shipping.

**In scope:**

- **Homepage content rewrite** (`app/views/home/show.html.erb`): hero with single clear value proposition, features grid (characters-level highlighting, private + shared notes, group Bible real-time collaboration, public notes with upvotes, bilingual KJV + RV1909, keyword + semantic search, dark mode, bilingual UI) — **only listing features that actually exist and work**, how-it-works section in ~3 steps, about section with mission/scope, donation CTA pointing at `/donate`. Bilingual copy in `config/locales/{en,es}.yml`. **No "open source" until the repo is public; no "accessibility-first" bare claim — "WCAG AA tested with axe" with the spec link if we want the credential.** Semantic search is only claimed if the embedding pserv is back up (tracked in memory as uncertain as of 2026-04-21); otherwise either drop it from the list or caveat with "semantic search beta."
- **Bitcoin donations** — `DonationsController` with:
  - `GET /donate` — page rendering: address QR, current Lightning invoice for fast-path donations (if OpenNode exposes), donation disclaimer, link to receipt-less anonymous flow.
  - `POST /donate` — create an OpenNode invoice via their REST API, redirect to the hosted checkout.
  - `POST /webhooks/opennode` — receive paid/confirmed webhook, verify signature with the OpenNode webhook secret, record the donation, send a thank-you email if the donor provided one.
  - `Donation` model: `amount_satoshis`, `status` enum (`pending | confirmed | expired`), `opennode_charge_id`, `donor_email` (optional), `confirmed_at`.
- **SMTP wiring** — `config/environments/production.rb` `config.action_mailer.smtp_settings` pointing at Resend's SMTP endpoint. Credentials in Rails credentials (`smtp.resend` with `api_key`). Devise password reset confirmed working end-to-end via a request spec that exercises the full mailer path. `default_url_options` host already set to `bible-together.org` (Sprint 12 deploy prep) so reset links are correct.
- **Donation confirmation mailer** (`DonationMailer#thank_you`) — renders plain-text + HTML, uses the same modern tokens the rest of the app does (self-hosted fonts won't load in email clients, fall back to system ui-sans / ui-serif).

**Out of scope (defer to Sprint 16 or later):**

- Donor tracking dashboard, donor wall, receipt PDFs — Option (c) scope from earlier planning, explicitly deferred.
- Additional mailer templates beyond password reset + donation thank-you (confirmation, unlock, email-changed — Devise's default templates render in the new auth_shell but plain-text fallbacks aren't re-themed).
- Custom error pages (`public/400.html`, `404.html`, etc.) — still Rails-stock from the audit.
- BTCPay Server self-hosted migration — see tradeoff in the decisions block.
- Any changes to the 571-spec suite structure; this sprint adds specs, doesn't restructure.

**TDD approach per feature** (explicit, per new Rule 9):

- **Donation flow**: spec-first at the request level using WebMock — same pattern as `EmbeddingService` specs from Sprint 9. Stub the OpenNode create-charge endpoint, assert the `DonationsController#create` builds the right request payload and handles both 2xx (stores the charge id, redirects to hosted checkout) and error responses. Webhook receiver gets its own request spec: signature verification (valid / invalid / missing), status transitions (`pending → confirmed` on `charge.confirmed` event, `pending → expired` on `charge.expired`), idempotency (duplicate webhook doesn't double-count). System spec on `/donate` asserts the page renders with the address + QR and the donate form submits.
- **Mailer**: ActionMailer preview specs for `DonationMailer#thank_you` (renders in both locales without errors), delivery spec (`expect { DonationMailer.thank_you(...).deliver_now }.to change { ActionMailer::Base.deliveries.size }.by(1)`), Devise reset-password request spec that exercises the full send-reset-instructions path and asserts the delivered email subject + body + reset-link host.
- **Homepage**: system specs for every CTA — "Read the Bible" lands on `/public/bible/kjv/gen/1`, "Donate" lands on `/donate`, any feature-card link lands where it claims. Request spec for i18n coverage in both locales (assertions on key English phrases under `?locale=en` and Spanish equivalents under `?locale=es`) so a missing translation can't ship silently.

**Operational baseline:** this is the first sprint where Rule 9 applies from commit 1. Every UI-touching commit ships its matching system spec in the same commit; branch protection on main refuses non-green pushes. `spec/rails_helper.rb` now rebuilds Tailwind before any system-spec run (commit 276cc85), so stale-CSS false-green can't re-occur. If anything in this sprint requires skipping a system-spec update, that's a planning error — flag it and stop, don't ship.

**Known fallout / risks:**

- Resend DNS verification requires access to bible-together.org's DNS provider. Not a code-side blocker but a deploy-readiness gate.
- OpenNode merchant signup requires KYC that could take days to approve. The donation flow can be coded and tested against a mocked OpenNode API before that's resolved.
- If the embedding pserv is still down (uncertain per 2026-04-21 decisions), any homepage mention of semantic search is a lie until fixed. Easiest: omit semantic-search from the features grid, re-add in a follow-up when the pserv is verified live.
- Donation page copy is a jurisdictional minefield (see the deferred blocker). Draft with cautious generic language; legal review before `/donate` goes live.

**Confidence:** high on ordering, scope, and the spec patterns (all lifted from Sprints 8-13's established shapes). Medium on the OpenNode integration specifics — I haven't read their docs closely yet; actual implementation commits will open with a doc-read pass. Medium on homepage copy — writing is iterative, expect a follow-up polish commit after the user reads it in context.
